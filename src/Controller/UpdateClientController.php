...
class UpdateClientController extends AbstractController
{
    ...
    public function __invoke(UserRepository $ur , Request $request , RankRepository $rankRepository){
        ...
        if (!$user instanceof User) {
            ...
            $insert = $ur->findOneBy(array('id' => $content['id']));
            if (! $insert instanceof User) {
                return $this->json_response('401', 'user not found');
            }
            ...
            if ($username_test instanceof User and $username_test->getId() != $content['id']) {
                return $this->json_response('401', 'username déja utilisé');
            }


            $email_test =  $ur->findOneBy(array('email' => $content['email']));
            if ($email_test instanceof User  and $email_test->getId() != $content['id']) {
